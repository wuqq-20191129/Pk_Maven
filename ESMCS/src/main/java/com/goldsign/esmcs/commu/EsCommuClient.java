...
public class EsCommuClient {
    ...
    private static void readBytes(byte[] buff, int off, int len)
            throws IOException, MessageException, InterruptedException {
        int readTotal = 0;
        int restTotal = 0;
        if (!stopReader) {
...
//            if(rest!=0){
//               byte[] buffRest=new byte[rest];
//                restTotal = in.read(buffRest, off, len);
//                logger.info("剩余："+restTotal);
//                logger.info("接收数据："+CharUtil.byteToHex(buffRest));
//            }
//
            if (readTotal != len) {
                resultCode = "1201";
...
//                }else{
//                    if(isConnected()){
//                        readTotal=readTotal+in.read(buff,readTotal,available);
//                        if (readTotal != len) {
//                            resultCode = "1201";
//                            throw new MessageException(resultCode);
//                        }
